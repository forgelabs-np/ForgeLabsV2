...
function usePreloadImages(images: (string | undefined)[]) {
  useEffect(() => {
    images.forEach((src) => {
      if (!src) return;
      const img = new Image()
...
export function InteractiveTestimonials() {
  ...
  usePreloadImages(testimonials.map((t) => t.avatar?.imageUrl))
    ...
    >
      ...
      >
        {testimonials.map((t, i) => (
          ...
          >
            {t.avatar && <img src={t.avatar.imageUrl} alt={t.author} className="w-full h-full object-cover" />}
          </motion.div>
        ))}
      </motion.div>
